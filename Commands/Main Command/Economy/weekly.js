...
module.exports = {
    ...
    callback: (message, args) => {
        ...
        const dailyTime = db.fetch(`week-time_${User.id}`)
    
        if (dailyTime !== null && timeout - (Date.now() - dailyTime) > 0) {
            const timeleft = ms(timeout - (Date.now() - dailyTime))
    
            const Embed = new Discord.MessageEmbed()
            message.channel.send(`**🕐 |** Đã nhận weekly, xin hãy quay lại sau **${timeleft.days} ngày ${timeleft.hours} giờ ${timeleft.minutes} phút ${timeleft.seconds} giây!** | ${User.displayName}`)
        } else {
            if (final === 'Thành công') {
                db.add(`money_${User.id}`, amount)
                message.channel.send(`**✅ |** Đã gửi \`$${amount}\` cho <@${User.user.id}>`)
                db.set(`week-time_${User.id}`, Date.now())
            }
        }
    }
}